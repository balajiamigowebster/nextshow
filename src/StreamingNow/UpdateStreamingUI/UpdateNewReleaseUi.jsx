...
const UpdateNewReleaseUi = ({ newReleaseStreaming = [] }) => {
  const CURRENT = new Date();
  const CURRENT_YEAR = CURRENT.getFullYear();
  ...
  useEffect(() => {
    const list = Array.isArray(newReleaseStreaming) ? newReleaseStreaming : [];
    if (list.length > 0) {
      const MONTH_NAMES = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];
  ...
  // API CALLS
  // =========================
  const { data: availableDates, isLoading: isDateLoading } =
    useMovieDateAvailability({
      year: selectedYear,
      month: selectedMonth,
...
export default UpdateNewReleaseUi;
