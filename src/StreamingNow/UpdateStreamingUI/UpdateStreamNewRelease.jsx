...
// Props: newReleases = []
// ─────────────────────────────────────────
const UpdateStreamNewRelease = ({ newReleaseStreaming = [] }) => {
  console.log("New Releases", newReleaseStreaming);
  ...
  return (
    <div className="px-1 relative">
      {newReleaseStreaming.length > 4 && (
        <button
          onClick={() => openDrawer("New Release", newReleaseStreaming)}
          className="
      absolute
      -top-7
      right-2
...
text-[10px] uppercase tracking-widest font-bold text-gray-500 hover:text-gray-300 cursor-pointer transition-colors
    "
      ...
      {newReleaseStreaming.length > 0 ? (
        <div className="space-y-8">
          <CarouselRow
            title="New Release"
            movies={newReleaseStreaming}
            onSeeAll={() => openDrawer("New Release", newReleaseStreaming)}
          />
        </div>
      ) : (
        <EmptyState />
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 nr-custom-scrollbar">
      ...
      <style jsx="true">{`
        .nr-custom-scrollbar::-webkit-scrollbar {
          width: 4px;
        }
        .nr-custom-scrollbar::-webkit-scrollbar-track {
          background: #1a1a1a;
          border-radius: 10px;
        }
        .nr-custom-scrollbar::-webkit-scrollbar-thumb {
          background: #333;
          border-radius: 10px;
        }
        .nr-custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: #ff7d3b;
        }
      `}</style>
    </div>
...
export default UpdateStreamNewRelease;
