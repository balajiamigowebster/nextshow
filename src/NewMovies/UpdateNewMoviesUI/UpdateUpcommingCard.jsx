...
// ─────────────────────────────────────────
const makeMarquee = (animClass, dep) => {
  return ({ children, deps }) => {
    ...
    return (
      <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
        <div
          ref={contentRef}
          className={shouldAnimate ? animClass : "inline-flex"}
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={
          shouldAnimate && isHovered ? "nm-director-marquee" : "inline-flex"
        }
        style={{ "--distance": `${distance}px` }}
      >
        <span
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={
          shouldAnimate && isHovered ? "nm-genre-marquee" : "inline-flex"
        }
        style={{ "--distance": `${distance}px` }}
      >
        {genres.map((genre) => (
...
const TitleMarquee = ({ title, isHovered }) => {
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={shouldAnimate && isHovered ? "nm-title-marquee" : ""}
        style={{ "--distance": `${distance}px` }}
      >
        <h3
...
const TheatreRunningBadge = ({ movie }) => {
  const isRunning =
    movie?.releaseMode === "THEATRICAL" &&
    movie?.isTheatreReleased === true &&
  ...
  return (
    ...
    >
      ...
      <Film size={7} className="md:w-[9px] md:h-[9px]" />
      In Theatre
    </span>
  );
};
...
//   trailerUrl, bannerImage, releaseDate, theatricalReleaseDate,
//   averageRating, streamType, isStreamingReleased, isNew }
// ─────────────────────────────────────────
const UpdateUpcommingCard = ({ movie = {}, title }) => {
  ...
  // New Movies uses theatricalReleaseDate or releaseDate
  const releaseLabel = movie.theatreReleaseDate;

  // Badge logic for theatrical movies
  const isNewRelease =
    movie.streamType === "NEW_RELEASE" || movie.isNew === true;
  const isUpcoming = movie.streamType === "UPCOMING" && !movie.isNew;

  const showRating = !!movie.averageRating;
  ...
  const shouldShowCountdown =
    movie?.releaseMode === "THEATRICAL" &&
    movie?.movieStatus === "WAITING" &&
    movie?.streamType === "UPCOMING" &&
    !movie?.isTheatreReleased &&
    movie?.theatreReleaseDate !== "TBA";
      ...
      "
        ...
        {showRating && (
          ...
          >
            ...
            <span className="nm-rating-score text-[12px] md:text-[13px] whitespace-nowrap">
              {movie.averageRating}
            </span>
          </span>
        )}
  ...
  "
      ...
      "
              ...
              <TheatreRunningBadge movie={movie} />
            </div>
      ...
      "
              >
                ...
                <LiveRelativeDate targetDate={movie.theatreReleaseDate} />
              </span>
            )}
          </div>
        )}
    ...
    text-zinc-400"
        ...
        <style jsx="true">{`
          /* ── Marquee keyframes (nm- prefixed to avoid conflicts) ── */
          @keyframes nmDirectorMarquee {
            0% {
          ...
          .nm-director-marquee {
            display: inline-flex;
            width: max-content;
            animation: nmDirectorMarquee 8s ease-in-out infinite;
          }

          @keyframes nmCastMarquee {
            0% {
              transform: translateX(0);
            }
            45% {
          ...
          .nm-cast-marquee {
            display: inline-flex;
            width: max-content;
            animation: nmCastMarquee 35s ease-in-out infinite;
          }

          @keyframes nmGenreMarquee {
            0% {
              transform: translateX(0);
            }
            45% {
          ...
          .nm-genre-marquee {
            display: inline-flex;
            width: max-content;
            animation: nmGenreMarquee 15s ease-in-out infinite;
          }

          @keyframes nmTitleMarquee {
            0% {
              transform: translateX(0);
            }
            45% {
          ...
          .nm-title-marquee {
            ...
            width: max-content;
            animation: nmTitleMarquee 10s ease-in-out infinite;
          }
          ...
          .new-release-badge .shine {
            ...
            animation: goldShine 10s cubic-bezier(0.4, 0, 0.6, 1) infinite;
            pointer-events: none;
          }
          @keyframes goldShine {
            0% {
...
export default UpdateUpcommingCard;
