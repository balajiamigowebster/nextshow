    ...
    <div className="relative group mb-8 last:mb-0">
      <div className="relative overflow-hidden px-1 py-1">
        ...
        <Swiper
          ...
          className="nm-swiper !overflow-visible"
        >
          {movies.map((movie) => (
...
// ─────────────────────────────────────────
// MAIN SECTION
// Props: movies = [] (new theatrical movies array)
// ─────────────────────────────────────────
const UpdateNewMoviesUpcoming = ({ upcomingNewMovies = [] }) => {
  ...
  return (
    <div className="px-1 relative">
      {upcomingNewMovies.length > 4 && (
        <button
          onClick={() => openDrawer("Upcoming New Movies", upcomingNewMovies)}
          className="
      absolute
      -top-7
      right-2
...
text-[10px] uppercase tracking-widest font-bold text-gray-500 hover:text-gray-300 cursor-pointer transition-colors
    "
      ...
      {upcomingNewMovies.length > 0 ? (
        <div className="space-y-8">
          <CarouselRow
            title="Upcoming"
            movies={upcomingNewMovies}
            onSeeAll={() => openDrawer("Upcoming", upcomingNewMovies)}
          />
        </div>
      ) : (
        <EmptyState />
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 nm-custom-scrollbar">
      ...
      <style jsx="true">{`
        ...
        .nm-custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: #ff7d3b;
        }
      `}</style>
    </div>
...
export default UpdateNewMoviesUpcoming;
