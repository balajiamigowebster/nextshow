...
const DirectorMarquee = ({ director }) => {
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={shouldAnimate ? "trailer-director-marquee" : "inline-flex"}
...
const CastMarquee = ({ cast = [] }) => {
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={shouldAnimate ? "trailer-cast-marquee" : "inline-flex"}
        style={{ "--distance": `${distance}px` }}
      >
        {cast.map((actor) => (
          <span
...
const TitleMarquee = ({ title }) => {
  ...
  return (
    <div ref={containerRef} className="overflow-hidden min-w-0 w-full">
      <div
        ref={contentRef}
        className={shouldAnimate ? "trailer-title-marquee" : ""}
        style={{ "--distance": `${distance}px` }}
      >
...
const ScheduledCardBody = ({ trailer, statusLabel, statusClass }) => {
  ...
  return (
    <div className="flex flex-col bg-zinc-900 border-t border-zinc-800 px-2.5 pt-2 pb-2.5 gap-2">
      {/* Coming Soon Banner */}
...
// ============== Published Card Body ==============
const PublishedCardBody = ({ trailer, director, castList = [] }) => {
  ...
  return (
    <div className="flex flex-col bg-zinc-900 border-t border-zinc-800 px-2.5 pt-2 pb-2.5 gap-2">
      ...
      <div className="overflow-hidden w-full">
        <TitleMarquee title={trailer.title || trailer.youtubeTitle} />
      </div>
      ...
      <div className="flex flex-col gap-1 w-full">
        <div className="grid grid-cols-[30px_1fr] gap-2 items-start text-[10px]">
          <div className="text-zinc-600 flex items-center gap-1 font-semibold tracking-wider mt-0.5">
            Dir
          </div>
        ...
        <div className="grid grid-cols-[38px_1fr] gap-1 items-start text-[10px]">
          ...
          <div className="overflow-hidden min-w-0">
            <CastMarquee cast={castList.length ? castList : ["-- / --"]} />
          </div>
        </div>
      </div>
    </div>
...
const TrailerCard = ({ trailer, title, onClick }) => {
  ...
  return (
    <div className="relative flex flex-col rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900 flex-shrink-0 select-none transition-all duration-500 ease-out hover:border-zinc-500 hover:ring-2 hover:ring-zinc-500/20 hover:shadow-[0_0_20px_rgba(161,161,170,0.15)]">
      {/* ── Thumbnail Area ── */}
